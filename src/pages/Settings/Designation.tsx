...
const DeleteConfirmationDialog = ({ open, onClose, onConfirm }: { open: boolean; onClose: () => void; onConfirm: () => void }) => (
  ...
  >
    ...
    <DialogContent>Are you sure you want to delete this Desgnation?</DialogContent>
    <DialogActions>
...
export default function Designation() {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    if (validate()) {
      console.log('Form Submitted:', { designation: formData.designation.value, status: formData.status.value });
      handlePopupClose();
    }
  };
  ...
  const handleEdit = (row: any) => {
    setIsEditing(true);
    setFormData({
      designation: { ...formData.designation, value: row.name },
  ...
  const columns = useMemo(
    () => [
      ...
      { header: 'Desgnation', accessorKey: 'name' },
      {
        header: 'Status',
        accessorKey: 'status',
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{isEditing ? 'Edit Desgnation' : 'Add New Desgnation'}</DialogTitle>
        <Container>
          <form onSubmit={handleSubmit} noValidate>
            <DialogContent>
              <Grid item marginBottom={2} xs={12}>
                <CommonInputField inputProps={formData.designation} onChange={handleInputChange} />
              </Grid>
