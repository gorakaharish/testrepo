...
const DeleteConfirmationDialog = ({ open, onClose, onConfirm }: { open: boolean; onClose: () => void; onConfirm: () => void }) => (
  ...
  >
    ...
    <DialogContent>Are you sure you want to delete this Property Type?</DialogContent>
    <DialogActions>
...
export default function PropertyType() {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    if (validate()) {
      console.log('Form Submitted:', { propertytype: formData.propertytype.value, status: formData.status.value });
      handlePopupClose();
    }
  };
  ...
  const handleEdit = (row: any) => {
    setIsEditing(true);
    setFormData({
      propertytype: { ...formData.propertytype, value: row.name },
  ...
  const data: any = [
    { name: 'Land', status: 'Active' },
    { name: 'Plot', status: 'Block' },
    { name: 'House', status: 'Active' },
    { name: 'Apertmant', status: 'Block' },
    { name: 'Plot', status: 'Active' },
    { name: 'House', status: 'Block' },
    { name: 'Land', status: 'Active' },
    { name: 'Apertmant', status: 'Block' }
  ];
  ...
  const columns = useMemo(
    () => [
      ...
      { header: 'Property Type', accessorKey: 'name' },
      {
        header: 'Status',
        accessorKey: 'status',
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{isEditing ? 'Edit Property Type' : 'Add New Property'}</DialogTitle>
        <Container>
          <form onSubmit={handleSubmit} noValidate>
            <DialogContent>
              <Grid item marginBottom={2} xs={12}>
                <CommonInputField inputProps={formData.propertytype} onChange={handleInputChange} />
              </Grid>
      ...
      >
        <Alert onClose={handleSnackbarClose} severity="success">
          Successfully deleted the Property Type!
        </Alert>
      </Snackbar>
