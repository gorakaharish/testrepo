...
export default function Teams() {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    if (validate()) {
      console.log('Form Submitted:', { teams: formData.teams.value, status: formData.status.value });
      handlePopupClose();
    }
  };
  ...
  const handleEdit = (row: any) => {
    setIsEditing(true);
    setFormData({
      teams: { ...formData.teams, value: row.name },
  ...
  const data: any = [
    { name: 'Land Team', status: 'Active' },
    { name: 'Plot Team', status: 'Block' },
    { name: 'House Team', status: 'Active' },
    { name: 'Apertmant Team', status: 'Block' },
    { name: 'Plot Team', status: 'Active' },
    { name: 'House Team', status: 'Block' },
    { name: 'Land Team', status: 'Active' },
    { name: 'Apertmant Team', status: 'Block' }
  ];
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{isEditing ? 'Edit Teams' : 'Add New Teams'}</DialogTitle>
        <Container>
          <form onSubmit={handleSubmit} noValidate>
            <DialogContent>
              <Grid item marginBottom={2} xs={12}>
                <CommonInputField inputProps={formData.teams} onChange={handleInputChange} />
              </Grid>
