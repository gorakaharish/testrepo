...
const DeleteConfirmationDialog = ({ open, onClose, onConfirm }: { open: boolean; onClose: () => void; onConfirm: () => void }) => (
  ...
  >
    ...
    <DialogContent>Are you sure you want to delete this Property Category?</DialogContent>
    <DialogActions>
...
export default function PropertyCategory() {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    if (validate()) {
      console.log('Form Submitted:', { propertycateogy: formData.propertycateogy.value, status: formData.status.value });
      handlePopupClose();
    }
  };
  ...
  const handleEdit = (row: any) => {
    setIsEditing(true);
    setFormData({
      propertycateogy: { ...formData.propertycateogy, value: row.name },
  ...
  const data: any = [
    { name: 'Lands', status: 'Active' },
    { name: 'Plots', status: 'Block' },
    { name: 'Houses', status: 'Active' },
    { name: 'Apertmants', status: 'Block' },
    { name: 'Plots', status: 'Active' },
    { name: 'Houses', status: 'Block' },
    { name: 'Lands', status: 'Active' },
    { name: 'Apertmants', status: 'Block' }
  ];
  ...
  const columns = useMemo(
    () => [
      ...
      { header: 'Property Category', accessorKey: 'name' },
      {
        header: 'Status',
        accessorKey: 'status',
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{isEditing ? 'Edit Property Category' : 'Add New Property Category'}</DialogTitle>
        <Container>
          <form onSubmit={handleSubmit} noValidate>
            <DialogContent>
              <Grid item marginBottom={2} xs={12}>
                <CommonInputField inputProps={formData.propertycateogy} onChange={handleInputChange} />
              </Grid>
            ...
            <DialogActions sx={{ margin: '0.7rem' }}>
      ...
      >
        <Alert onClose={handleSnackbarClose} severity="success">
          Successfully deleted the Property Category!
        </Alert>
      </Snackbar>
